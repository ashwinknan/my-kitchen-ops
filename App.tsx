...
import { Search, Utensils, Calendar, ShoppingCart, Settings2, Flame, RefreshCcw, Check, Trash2, Plus, Minus, ChevronDown, ChevronUp, Database, WifiOff, AlertCircle, Terminal, Activity, ShieldAlert, X, Key } from 'lucide-react';
...
import { auth, getAuthError } from './firebase';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-400 font-medium animate-pulse">Loading Kitchen...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-xl mx-auto pb-32 px-4 pt-4 min-h-screen">
      ...
      <header className="mb-6 flex items-center justify-between">
        ...
        <button onClick={() => setShowDebug(!showDebug)} className="p-2 text-slate-400">
          <Terminal size={18} />
        </button>
      </header>

      {showDebug && (
        <div className="mb-6 p-4 bg-slate-900 text-green-400 font-mono text-[10px] rounded-2xl overflow-x-auto shadow-xl">
           <p className="mb-2 text-slate-500 border-b border-slate-800 pb-1 flex justify-between">
            <span>System Diagnostics</span>
            <span className="text-orange-500">ROOT@HOME</span>
           </p>
           <p>Recipes: {recipes.length}</p>
           <p>Auth: {auth.currentUser?.uid || 'NONE'}</p>
           <p>Status: {process.env.API_KEY ? 'API_KEY_LOADED' : 'KEY_MISSING'}</p>
        </div>
      )}
          ...
          <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => setCookCount(Math.max(1, cookCount - 1))} className="w-8 h-8 rounded-full border border-slate-100 flex items-center justify-center bg-slate-50 text-slate-600"><Minus size={14} /></button>
                  <span className="text-lg font-bold w-4 text-center">{cookCount}</span>
                  <button onClick={() => setCookCount(Math.min(4, cookCount + 1))} className="w-8 h-8 rounded-full border border-slate-100 flex items-center justify-center bg-slate-50 text-slate-600"><Plus size={14} /></button>
                </div>
              </div>
              <div className="space-y-2">
                <span className="text-xs text-slate-400 font-medium">Stove Burners</span>
                <div className="flex items-center gap-3">
                  <button onClick={() => setStoveCount(Math.max(1, stoveCount - 1))} className="w-8 h-8 rounded-full border border-slate-100 flex items-center justify-center bg-slate-50 text-slate-600"><Minus size={14} /></button>
                  <span className="text-lg font-bold w-4 text-center">{stoveCount}</span>
                  <button onClick={() => setStoveCount(Math.min(6, stoveCount + 1))} className="w-8 h-8 rounded-full border border-slate-100 flex items-center justify-center bg-slate-50 text-slate-600"><Plus size={14} /></button>
                </div>
              </div>
            </div>
            ...
            <button 
              ...
              className="w-full mt-6 py-4 bg-orange-500 hover:bg-orange-600 disabled:bg-slate-100 disabled:text-slate-300 text-white font-bold rounded-2xl transition-all shadow-lg shadow-orange-500/20 flex items-center justify-center gap-2 text-lg"
            >
              {opsLoading ? <RefreshCcw className="animate-spin" size={20} /> : <Flame size={20} />}
              {opsLoading ? 'Planning...' : 'Plan My Cooking'}
            </button>
          </div>
          ...
          {optimizedResult && (
            <div id="ops-results" className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-500">
              <div className="flex justify-between items-center px-2">
                <h3 className="font-bold text-slate-900">Your Cooking Step-by-Step</h3>
                <span className="text-xs bg-slate-200 text-slate-600 px-3 py-1 rounded-full font-bold">{optimizedResult.totalDuration} Min Total</span>
              </div>
                ...
                {optimizedResult.timeline.map((step, i) => (
                  <div key={i} className="flex gap-4">
                    <div className="flex flex-col items-center">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm shrink-0 border-2 ${step.isParallel ? 'bg-orange-500 text-white border-orange-400' : 'bg-white text-slate-300 border-slate-100'}`}>
                        {step.timeOffset}'
                      </div>
                    ...
                    <div className="bg-white p-4 rounded-2xl border border-slate-100 flex-1 shadow-sm">
                      <div className="flex justify-between items-start mb-2">
                         ...
                         {step.isParallel && <span className="text-[8px] font-black text-green-500 bg-green-50 px-2 py-0.5 rounded border border-green-100">MULTI-TASK</span>}
                      </div>
                      <p className="text-sm font-bold text-slate-800 leading-snug">{step.action}</p>
                      <div className="mt-2 flex gap-2">
                        {step.involvedRecipes.map((r, ri) => (
                          <span key={ri} className="text-[9px] text-slate-400 font-bold uppercase">{r}</span>
                        ))}
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">Days to Plan</span>
                <span className="text-orange-500 font-bold">{plannerDays}</span>
              </div>
              <input 
            ...
            <div className="space-y-2">
              <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">What's in your fridge?</span>
              <textarea 
                placeholder="e.g. Leftover chicken, carrots, broccoli..." 
                className="w-full p-4 bg-slate-50 border-none rounded-2xl h-24 focus:ring-2 focus:ring-orange-500 transition-all text-sm font-medium"
                value={fridgeVeggies}
            ...
            <button 
              ...
              className="w-full py-4 bg-slate-900 text-white font-bold rounded-2xl flex items-center justify-center gap-2 shadow-lg"
            >
              {plannerLoading ? <RefreshCcw className="animate-spin" size={20} /> : <Calendar size={20} />}
              {plannerLoading ? 'Creating Plan...' : 'Suggest a Menu'}
            </button>
          </div>
          ...
          {mealPlan.length > 0 && (
            <div className="space-y-4">
              {mealPlan.map((day, idx) => (
                <div key={idx} className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
                  <div className="bg-slate-50 px-4 py-2 border-b border-slate-100 flex justify-between items-center">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Day {idx + 1}</span>
                  </div>
                  <div className="divide-y divide-slate-50">
                    {['breakfast', 'lunchDinner', 'snack'].map((m) => (
                      <div key={m} className="p-4">
                        <span className="text-[10px] text-orange-500 font-bold uppercase block mb-1">{m}</span>
              ...
              <button 
                ...
                className="w-full py-5 bg-orange-500 text-white font-bold rounded-2xl shadow-lg mt-4 flex items-center justify-center gap-2"
              >
                <ShoppingCart size={20} /> Generate Shopping List
              </button>
            </div>
          )}
        </div>
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 space-y-6">
            ...
            <div className="space-y-2">
              {shoppingList.map((item, i) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center ${item.checked ? 'bg-green-500 border-green-500' : 'bg-white border-slate-200'}`}>
                  ...
                  <span className="text-[10px] font-bold bg-white px-2 py-1 rounded-lg border border-slate-100">{item.value} {item.unit}</span>
                </div>
              ))}
              {shoppingList.length === 0 && (
                <div className="text-center py-12 text-slate-300 text-sm font-medium">No items in your list yet.</div>
              )}
            </div>
          </div>
        </div>
      ...
      <nav className="fixed bottom-6 left-4 right-4 bg-slate-900/95 backdrop-blur-xl py-4 px-8 flex justify-between items-center z-50 shadow-2xl rounded-3xl border border-white/10">
        {[
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1 transition-all ${activeTab === btn.id ? 'text-orange-500' : 'text-slate-500 hover:text-slate-300'}`}
          >
